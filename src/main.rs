...
use d2d::{create_d2d_factory, create_d2d_device};
use d3d::create_d3d_device;
use interop::{shutdown_dispatcher_queue_controller_and_wait, create_dispatcher_queue_controller_for_current_thread};
use numerics::ToVector2;
use window::Window;
use windows::{
    core::{Result, Error, HRESULT, HSTRING},
    Foundation::Numerics::{Vector2, Vector3, Matrix3x2},
    Win32::{
        Foundation::{HWND, RECT},
        System::WinRT::{RoInitialize, RO_INIT_SINGLETHREADED},
        UI::WindowsAndMessaging::{DispatchMessageW, GetMessageW, TranslateMessage, MSG}, Graphics::{Direct3D11::{D3D11_CREATE_DEVICE_BGRA_SUPPORT, D3D11_CREATE_DEVICE_DEBUG}, Direct2D::{D2D1_FACTORY_OPTIONS, D2D1_DEBUG_LEVEL_INFORMATION, ID2D1DeviceContext, Common::{D2D1_COLOR_F, D2D_POINT_2F}, D2D1_DRAW_TEXT_OPTIONS_NONE}, DirectWrite::{DWriteCreateFactory, DWRITE_FACTORY_TYPE_SHARED, IDWriteFactory, IDWriteFontCollection, DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, DWRITE_OVERHANG_METRICS}},
    },
    UI::{Composition::{Compositor, AnimationIterationBehavior}, Color}, w, Graphics::{SizeInt32, DirectX::{DirectXPixelFormat, DirectXAlphaMode}},
};
...
fn run() -> Result<()> {
    ...
    let text_format = unsafe {
        ...
        let locale = w!("en-us");
        dwrite_factory.CreateTextFormat(font_name, &font_collection, DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, 36.0, locale)?
    };
    ...
    let metrics = unsafe {
        text_layout.GetOverhangMetrics()?
    };
    ...
    let text_size = SizeInt32{ Width: text_rect.right - text_rect.left, Height: text_rect.bottom - text_rect.top};
    ...
    visual.SetSize(Vector2::new(text_size.Width as f32, text_size.Height as f32))?;
    root.Children()?.InsertAtTop(&visual)?;

    // Setup our basic brush
    let surface = comp_graphics.CreateDrawingSurface2(text_size, DirectXPixelFormat::A8UIntNormalized, DirectXAlphaMode::Premultiplied)?;
    let basic_brush = compositor.CreateSurfaceBrushWithSurface(&surface)?;
    draw_into_surface(&surface, |d2d_context: &ID2D1DeviceContext, offset| {
        unsafe {
            ...
            let d2d_brush = {
                ...
                d2d_context.CreateSolidColorBrush(&color, None).expect("Failed to create color brush!")
            };

            d2d_context.Clear(Some(&D2D1_COLOR_F{ r: 0.0, g: 0.0, b: 0.0, a: 0.0 }));
            d2d_context.DrawTextLayout(D2D_POINT_2F{ x: 0.0, y: 0.0 }, &text_layout, &d2d_brush, D2D1_DRAW_TEXT_OPTIONS_NONE);
        }
    })?;

    // Create our mask brush
    let mask_brush = compositor.CreateMaskBrush()?;
    let text_color_brush = compositor.CreateColorBrushWithColor(Color { A: 255, R: 255, G: 0, B: 0})?;
    ...
    animation.InsertKeyFrame(0.0, Color{ A: 255, R: 255, G: 0, B: 0})?;
    animation.InsertKeyFrame(0.25, Color{ A: 255, R: 0, G: 255, B: 0})?;
    animation.InsertKeyFrame(0.5, Color{ A: 255, R: 0, G: 0, B: 255})?;
    animation.InsertKeyFrame(0.75, Color{ A: 255, R: 255, G: 255, B: 0})?;
    animation.InsertKeyFrame(1.0, Color{ A: 255, R: 255, G: 0, B: 0})?;
    ...
    border_brush.SetSource(&compositor.CreateColorBrushWithColor(Color{ A: 255, R: 255, G: 0, B: 0})?)?;
